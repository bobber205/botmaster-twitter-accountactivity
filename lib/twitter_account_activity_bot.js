...
class TwitterAccountActivityBot extends BaseBot {
  ...
  __processDirectMessageEvent(messages)  {
    const users_array = messages['users'];
    _.each(messages['direct_message_events'], (direct_message_entry) => {
      ...
      redisClient.get(redis_key, (err, master_question_object) => {
        if (master_question_object) {
          const original_message_text = direct_message_entry['message_create']['message_data']['text'];

          let dm_url = direct_message_entry.message_create.message_data.entities.urls[0];
          if (dm_url) {
            const message_text_urls_removed = _.replace(original_message_text, dm_url['url']);
            console.log("URL", dm_url['url']);
            console.log("message_text_urls_removed", message_text_urls_removed);
          } else {
            const message_text_urls_removed = original_message_text;
          }
          ...
          console.log('Master master_question_object', master_question_object);
          console.log('question_matched', question_matched);
          if (question_matched) {
            direct_message_entry['question_number'] = question_matched.question_number;
            direct_message_entry['correct'] = _.intersection(question_matched.answers, [message_text_urls_removed]).length > 0 ? '1' : '0'
