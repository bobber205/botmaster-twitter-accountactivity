...
class TwitterAccountActivityBot extends BaseBot {
        ...
        if (master_question_object) {
          ...
          if (question_matched) {
            ...
            if (reply_answer_status_to_user) {
              let message_to_user = was_correct == "1" ? `Your Answer for Question ${question_matched.question_number + 1} Was Correct! :D ${moment().format("x")}` : `Wow it’s too bad but your answer for ${question_matched.question_number + 1} was wrong! Check out the results and wait for the next question at https://twitter.com/chatbottrivia ${moment().format("x")}`;
  ...
  __logDirectMessageEvent(users_array, direct_message_entry, original_object) {
      ...
      if (_.includes(["christeso", "bobber205", "jingwait", "trailrunner_vip", "chatbottrivia"], sender_object.screen_name.toLowerCase())) {
