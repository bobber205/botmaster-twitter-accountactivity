        ...
        if (master_question_object) {
          ...
          if (question_matched) {
            ...
            if (reply_answer_status_to_user) {
              ...
              // let message_to_user = was_correct == "1" ? `Your Answer for Q${question_matched.question_number + 1} Was Correct! Check out the results and wait for the next question on our profile. ${moment().format("x")}` : `Wow it’s too bad but your answer for Q${question_matched.question_number + 1} was wrong! ${moment().format("x")}`;
