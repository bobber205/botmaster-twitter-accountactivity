    ...
    _.each(messages['direct_message_events'], (direct_message_entry) => {
      ...
      redisClient.get(redis_key, (err, master_question_object) => {
        if (master_question_object) {
          ...
          const question_index = questionHelpers.determineQuestionAnswered(answer_sent_time, master_question_object);
          console.log("Master master_question_object", master_question_object, master_question_object.length);
          console.log("Master master_question_object", question_index);
        }
      });
    });
  }
