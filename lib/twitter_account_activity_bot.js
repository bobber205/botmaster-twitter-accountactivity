...
redisClient.on('connect', function(err) {
  redisClient.getAsync(redisHelpers.getConfigurationKey()).then((config_json) => {
    ...
    console.log("CONFIG JSON IS", config);
...
class TwitterAccountActivityBot extends BaseBot {
  ...
  __processDirectMessageEvent(messages, request, response)  {
    ...
    _.each(events, (direct_message_entry) => {
      ...
      redisClient.getAsync(redis_key).then( (master_question_object) => {
  ...
  __sendDMToUser(recipient_id, message_text) {
    ...
    var image_id = process.env.MEDIA_ID_TEMP;
    let dm_payload = {
      event: {
        type: 'message_create',
        message_create: {
    ...
    if (image_id) {
      ...
      dm_payload.event.message_create.message_data.attachment.type = "media";
      dm_payload.event.message_create.message_data.attachment.media = {
        id: image_id
      }
    }
