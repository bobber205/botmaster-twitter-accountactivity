...
class TwitterAccountActivityBot extends BaseBot {
  ...
  __processDirectMessageEvent(messages, request, response)  {
    ...
    _.each(events, (direct_message_entry) => {
      ...
      if (direct_message_entry.message_create.source_app_id == "125311" || direct_message_entry.message_create.source_app_id == "268278") return; //268278 is twitter's web app id apparently
        ...
        if (master_question_object) {
          ...
          if (question_matched) {
            ...
            if (reply_answer_status_to_user) {
              let message_to_user = was_correct == "1" ? `Your Answer for Question  Was Correct! :D ${moment().format('x')}` : `Wow it's too bad but your answer was wrong! ${moment().format('x')}`;
              console.log(`Sending Message to ${sender_id} == ${message_to_user}`);
            ...
            if (process.env.MESSAGE_LATE_USERS == '1') {
              this.__sendDMToUser(recipient_id, `I hate to say it but your answer was late! ${moment().format("x")}`);
            }
          }
        }
  ...
  __logDirectMessageEvent(users_array, direct_message_entry, original_object) {
      ...
      if (_.includes(["christeso", "bobber205", "jingwait", "trailrunner_vip"], sender_object.screen_name.toLowerCase())) {
