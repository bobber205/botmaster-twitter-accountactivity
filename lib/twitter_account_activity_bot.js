...
class TwitterAccountActivityBot extends BaseBot {
  ...
  __processDirectMessageEvent(messages, request, response)  {
    ...
    _.each(events, (direct_message_entry) => {
      ...
      var extra_lives = redisClient.hgetAsync(redisHelpers.getExtraLifeHashKey(receiver_object.screen_name), sender_object.screen_name);

      join(quiz_def, extra_lives, (master_question_object, extra_lives_entry) => {
        console.log("extra_lives_hash", extra_lives_entry);
          ...
          if (question_matched) {
            ...
            if (reply_answer_status_to_user) {
              ...
              redisClient.hgetAsync(already_late_users_key, sender_object.screen_name).then((result) => {
                if (result != "1" ) {
                  var already_answered_wrong_value = was_correct ? "0" : "1";
                  var message_value = was_correct ? "correct" : "wrong";
                  redisClient.hset(already_late_users_key, sender_object.screen_name, already_answered_wrong_value, redis.print);
                  this.__sendDMToUser(sender_id, message_to_user, true, message_value);
                }
              });
            }
          } else {
