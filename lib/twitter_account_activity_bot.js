...
class TwitterAccountActivityBot extends BaseBot {
  ...
  constructor(settings, expressApp) {
    ...
    console.log("Twitter Client! ==>", this.twitterClient)
  }
  ...
  __createMountPoints(expressApp) {
    ...
    this.app.post('*', (req, res) => {
      ...
      this.__logWebhookEvent(req.body, req, res);
      // const entries = req.body.entry;
      // this.__emitEvents(req.body, req, res);
      res.sendStatus(200);
    });
  }
      ...
      redisClient.get(redis_key, (err, master_question_object) => {
        if (master_question_object) {
          ...
          if (question_matched) {
            ...
            direct_message_entry['correct'] = answer_matched_array.length > 0 ? '1' : '0'
            ...
            // this.__sendDMToUser(recipient_id, 'hi there duder');
          } else {
            ...
            if (process.env.MESSAGE_LATE_USERS == '1') {
              this.__sendDMToUser(recipient_id, "Sorry Your Answer Came In Too Late");
            }
          }
        }
    ...
    if (true && false) {
      ...
      dm_payload.event.message_create.message_data['quick_reply'] = {
        ...
        options: [
          {
            label: 'A: some Answer!',
            description: 'some answer A',
            meta_data: "Answer-A"
          },
          {
            label: 'B: some Answer!',
            description: 'some answer B',
            meta_data: "Answer-B"
          }
        ]
      }
  ...
  __logDirectMessageEvent(users_array, direct_message_entry, original_object) {
      ...
      if (_.includes(['christeso', 'bobber205', 'jingwait'], sender_object.screen_name.toLowerCase())) {
  ...
  __processTypingEvent(messages) {
    ...
    _.each(events, (event) => {
      let recipient_id = event.target.recipient_id;
      let recipient_id = event.target.sender_id;
      //first find the question that the timestamp qualifies for
      let recipient_user = messages.users[recipient_id];
      let sender_object = messages.users[sender_id];

      let timestamp = event.created_timestamp / 1000; //get rid of the ms2
      let redis_key = `Chirpify::Trivia::MasterObject::${recipient_user.screen_name}`;
      ...
      redisClient.get(redis_key, (err, master_question_object) => {
        ...
        let answers = question_matched.answers;
        ...
        let options = _.map(answers, (answer, index) => {
          return {
            label: answer,
            description: '',
