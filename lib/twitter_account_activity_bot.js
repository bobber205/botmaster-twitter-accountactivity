    ...
    _.each(messages['direct_message_events'], (direct_message_entry) => {
      ...
      redisClient.get(redis_key, (err, master_question_object) => {
        if (master_question_object) {
          const message_text = direct_message_entry['message_create']['message_data']['text'];
          ...
          if (question_matched) {
            ...
            direct_message_entry['correct'] = _.intersection(question_matched.answers, [message_text]).length > 0 ? '1': '0'
