...
class TwitterAccountActivityBot extends BaseBot {
  ...
  __processDirectMessageEvent(messages, request, response)  {
    ...
    _.each(events, (direct_message_entry) => {
      ...
      var extra_lives = redisClient.getAsync(redisHelpers.getExtraLifeHashKey(receiver_object.screen_name));

      join(quiz_def, extra_lives, (master_question_object, extra_lives_hash) => {
