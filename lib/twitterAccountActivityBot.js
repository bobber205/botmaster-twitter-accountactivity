...
class TwitterAccountActivityBot {
    ...
    join(get_stats_object_def, extra_lives_def, get_player_count_def).then((stats_object) => {
      ...
      if (link_state === "still_playing_reminder") {
        ...
        if (stats_object.quizzes_count == 0 || !stats_object.quizzes_count) {
          message = `
          Welcome @${sender_object.screen_name}, we’re glad you decided to play your first game! You’re the ${thize(player_count)} player in. We will be firing up the Questionator and you'll receive the first question here in your DMs.
        `;
        } else {
          message = `
            Welcome @${sender_object.screen_name}, this is your ${thize(stats_object.quizzes_count + 1)} game! You're the ${thize(player_count)} player registered for today's game. Your IQ Score is ${stats_object.percentile} and you've got ${extra_life_count} extra lives.\n\nWe will be firing up the Questionator and you'll receive the first question here in your DMs!
          `;
        }
  ...
  __processWebhookEvent(messages) {
    ...
    } else if (message_type != 'direct_message_indicate_typing_events' && message_type != 'direct_message_mark_read_events_XXX') {
