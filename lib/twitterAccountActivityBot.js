...
const valid_extra_life_hashtags = ['yougotanextralife'];
...
class TwitterAccountActivityBot {
  ...
  __processTweetCreateEvent(messages) {
    ...
    _.each(tweet_events, (entry) => {
      ...
      if (valid_bot_handles.includes(entry.user.screen_name.toLowerCase()) && has_extra_life_hashtags) {
        //means a bot tweeted
        _.each(users_mentioned, (user_data) => {
          ...
          redisClient.hincrby(redisHelpers.getExtraLifeHashKey(receiver_object.screen_name.toLowerCase()), user_screen_name.toLowerCase(), 1, redis.print);
  ...
  __validatePaypalEmailAddress(sender_object, receiver_object, email_address) {
    ...
    if (is_email_address) {
      ...
      sidekiqConnection.enqueue(
        'Chirpify::Workers::ProcessEmailEventWorker', [{
