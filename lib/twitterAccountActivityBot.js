...
class TwitterAccountActivityBot {
      ...
      join(quiz_def, extra_lives_def, iq_score_def, user_bot_status_def, has_used_extra_life_for_quiz_def, has_extra_life_waiting_to_be_awarded_def, get_answer_status_key_def,(whole_question_object, extra_life_redis_value, score, bot_state_status, has_used_extra_life_for_quiz, has_extra_life_waiting_to_be_awarded_value, get_answer_status_value) => {
        ...
        if (whole_question_object) {
          ...
          if ( (answered_on_time || answered_late_but_its_ok) && question_matched) {
           ...
           else if (_.intersection(["👍 i'm in", "ready", "👍 hell yeah"], [message_text_urls_removed.toLowerCase()]).length >= 1) {
          //  else if (_.some(valid_opt_in_message, (current) => {return current.toLowerCase().indexOf(message_text_urls_removed.toLowerCase()) > -1 || message_text_urls_removed.toLowerCase().indexOf(current.toLowerCase()) > -1 })) {
            ...
            this.__processUserHasOptedInEvent(sender_object, receiver_object, false);
           }
           else if (_.intersection(["👍 i'm still in"], [message_text_urls_removed.toLowerCase()]).length >= 1) {
          //  else if (_.some(valid_opt_in_message, (current) => {return current.toLowerCase().indexOf(message_text_urls_removed.toLowerCase()) > -1 || message_text_urls_removed.toLowerCase().indexOf(current.toLowerCase()) > -1 })) {
            console.log(`SENT STILL IN ${message_text_urls_removed} ${valid_opt_in_message}`);
            this.__processUserHasOptedInEvent(sender_object, receiver_object, true);
           }
  ...
  __processUserHasOptedInEvent(sender_object, receiver_object, still_in_state = false) {
    ...
    join(get_stats_object_def, extra_lives_def, get_player_count_def).then((stats_object) => {
      ...
      if (still_in_state) {
        message = `The next question will be arriving shortly. Thanks for continuing to play for IQ Score!`;
        // redisClient.set(redisHelpers.getLatestAnswerValueKey(sender_object.id), 'Z');
      }
  ...
  async __standardBotMenuResponse(recipient_id, sender_id, bot_owner_name) {
    ...
    if (dm_payload.event.message_create.message_data.quick_reply.options.length == 2) {
      dm_payload.event.message_create.text = "There’s a game in progress.\nWant to play?";
    }
