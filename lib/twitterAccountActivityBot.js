...
class TwitterAccountActivityBot {
  ...
  __processUserHasOptedInEvent(sender_object, receiver_object) {
    ...
    var get_player_count = redisClient.hlenAsync(redisHelpers.getPlayerListKey(receiver_object.screen_name.toLowerCase()));

    join(get_stats_object_def, extra_lives_def).then((stats_object, player_count) => {
