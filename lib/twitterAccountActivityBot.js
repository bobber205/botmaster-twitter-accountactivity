...
class TwitterAccountActivityBot {
      ...
      join(quiz_def, extra_lives_def, iq_score_def, user_bot_status_def, can_use_extra_life_def, has_extra_life_waiting_to_be_awarded_def, get_answer_status_key_def, get_answer_status_key_ttl_def, (whole_question_object, extra_life_redis_value, score, bot_state_status, can_use_extra_life_on_this_quiz, has_extra_life_waiting_to_be_awarded_value, get_answer_status_value, user_answer_ttl_value) => {
        ...
        if (whole_question_object) {
          ...
          if ( (answered_on_time || answered_late_but_its_ok) && question_matched) {
           ...
           else if (_.intersection(["👍 i'm in", "i'm in", "ready", "i’m in", "in"], [message_text_urls_removed.toLowerCase()]).length >= 1) {
  ...
  __processUserHasOptedInEvent(sender_object, receiver_object, link_state = false) {
    ...
    console.log(`${redisHelpers.getNextGameTimeKey(receiver_object.screen_name)} next game key`);

    join(get_stats_object_def, extra_lives_def, get_player_count_def, get_time_remaining_def).then((stats_object) => {
      ...
      var remaining_duration = moment.duration(moment.utc(next_game_time*1000).diff(moment()));
      ...
      var minutes_remaining = minutes = Math.floor((remaining_ms % (1000 * 60 * 60)) / (1000 * 60));
      ...
      var time_message_minutes = hours_remaining > 1 ? 'minutes': `minute`;
      ...
      if (link_state === "still_playing_reminder") {
        ...
        if (stats_object.quizzes_count == 0 || !stats_object.quizzes_count) {
          message = `
          Welcome @${sender_object.screen_name}, we’re glad you decided to play your first game! You’re the ${thize(player_count)} player in. The Questionator will send you the first question here in your DMs when the game begins!`;
        } else {
          message = `
            Welcome @${sender_object.screen_name}, this is your ${thize(stats_object.quizzes_count + 1)} game! You're the ${thize(player_count)} player registered for this next game. Your IQ Score is ${stats_object.percentile} and you've got ${extra_life_count} extra lives.\n\nThe Questionator will send you the first question here in your DMs when the game begins!
          `;
        }
