  ...
  __processTweetCreateEvent(messages) {
    ...
    _.each(tweet_events, (entry) => {
      ...
      if (is_valid_bot_handle && has_broadcast_hashtags) {
        let dm_text = entry.text.replace(valid_broadcast_hashtags[0], '');
        if (entry.extended_tweet) {
          dm_text = entry.extended_tweet.full_text.replace(valid_broadcast_hashtags[0], '');
        }
        dm_text = dm_text.replace(/(?:https?|ftp):\/\/[\n\S]+/g, '');
        redisClient.hkeysAsync(`Chirpify::IQTrivia::PlayingQuiz::${entry.user.screen_name.toLowerCase()}`).then((twitter_id) => {
          console.log(`Sending ${dm_text} to ${twitter_id}`);
          this.__sendDMToUser(user_id, entry.user.screen_name.toLowerCase(), dm_text, false, '', false);
        });
      }
