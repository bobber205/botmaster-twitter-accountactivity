...
class TwitterAccountActivityBot extends BaseBot {
  ...
  __sendUserIQScore(sender_id, sender_handle, bot_owner_name, user_iq_score) {
    ...
    redisClient.hgetAsync(redisHelpers.getIQStatsKey(), sender_handle).then((result) => {
      if (!result) {
        this.__sendDMToUser(sender_id, bot_owner_name, `Your IQ score isn't available until you've played your first game.`, false);
      } else {
