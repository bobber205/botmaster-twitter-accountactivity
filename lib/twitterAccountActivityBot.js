        ...
        if (whole_question_object) {
          ...
          if (question_matched) {
            ...
            if (reply_answer_status_to_user) {
              ...
              if (extra_life_used) message_to_user = 'Your Extra Life Has Been Used!';
              // redisClient.hgetAsync(already_late_users_key, sender_object.screen_name).then((answered_late_value) => {
              
              redisClient.hgetAsync(redisHelpers.getRespondedAnswerKey(receiver_object.screen_name, question_matched.question_number), sender_object.screen_name).then((already_responded_value) => {
                ...
                // if (answered_late_value != "1") {
                //   var already_answered_wrong_value = (was_correct | extra_life_used) == 1 ? "0" : "1";
                  
                //   var message_value_type = was_correct ? "correct" : "wrong";
                //   if (extra_life_used) message_value_type = "extra_life_used";
                  
                //   redisClient.hset(already_late_users_key, sender_object.screen_name, already_answered_wrong_value, redis.print);
                //   this.__sendDMToUser(sender_id, message_to_user, true, message_value_type);
                // }
              });
            }
           ...
           } else if (message_text_urls_removed == 'IQ Score') {
  ...
  __sendUserIQScore(sender_id, sender_handle) {
    ...
    redisClient.hgetAsync(redisHelpers.getIQStatsKey(), sender_handle).then((result) => {
      if (!result) {
        ...
        let total_count = result["total_question_count"];
        let right_count = result["right_answer_count"];
        this.__sendDMToUser(sender_id, `Your IQ score is ${JSON.stringify(result)} ${percentile}!\nYour Lifetime Score is: ${right_count}/${total_count}`, false);
      }
    });
  }
