      ...
      join(quiz_def, extra_lives_def, iq_score_def, user_bot_status_def, has_used_extra_life_for_quiz_def, has_extra_life_waiting_to_be_awarded_def, (whole_question_object, extra_life_redis_value, score, bot_state_status, has_used_extra_life_for_quiz, has_extra_life_waiting_to_be_awarded_value) => {
        ...
        if (whole_question_object) {
          ...
          if (question_matched && has_extra_life_waiting_to_be_awarded_value) {
            ...
            redisClient.hincrby(redisHelpers.getExtraLifeHashKey(receiver_object.screen_name.toLowerCase()), user_who_had_code_redeemed.screen_name.toLowerCase(), 1, redis.print);
            ...
            this.__sendDMToUser(user_who_had_code_redeemed_id, receiver_object.screen_name, `I see you were invited by a friend! What was their code?`, false, '', false);
  ...
  __parseFreeLifeCode(sender_object, receiver_object, code_submitted) {
    ...
    join(is_valid_code_def, has_redeemed_code_already_def, (is_valid_code_value, has_redeemed_code_already_value) => {
      ...
      if (code_submitted.toLowerCase() == sender_object.screen_name.toLowerCase()) {
        ...
        var code_value = `${code_submitted}@@@${is_valid_code_value}`; //@ can't be in twitter usernames
        redisClient.hincrby(redisHelpers.getExtraLifeHashKey(receiver_object.screen_name.toLowerCase()), sender_object.screen_name.toLowerCase(), 1, redis.print);
