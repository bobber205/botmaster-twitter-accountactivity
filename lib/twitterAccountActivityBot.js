...
class TwitterAccountActivityBot {
  ...
  __processReminderMenu(sender_object, receiver_object) {
      console.log(`${sender_object.screen_name} (${sender_object.id}) has brought up reminder menu!`);
      ...
      join(sub_status_def).then((sub_status) => {
        ...
        if (sub_status != "0" && sub_status) {
          message = "Looks like you are already subscribed to Twitter notifications. 👍";
        } else {
          message = 'Would you like to be reminded via Twitter when the game is about to start?';
        }
  ...
  __processSubscriberMenu(sender_object, receiver_object, message_text) {
      console.log(`${sender_object.screen_name} (${sender_object.id}) has brought up sub menu!`);
      var sender_id = sender_object.id;
      var sub_status_def = redisClient.hgetAsync(redisHelpers.getSubStatusKey(), sender_id);
      join(sub_status_def).then((sub_status) => {
        ...
        sidekiqConnection.enqueue(
          'Chirpify::Workers::GenericLoggerWorker', ['reminder_subscribers', JSON.stringify(payload), create_or_update], {
            queue: 'logging'
          }
        );
      });
  ...
  __processUserHasOptedInEvent(sender_object, receiver_object, link_state = false) {
    ...
    join(get_stats_object_def, extra_lives_def, get_player_count_def, get_time_remaining_def, is_opted_in_already_def).then((stats_object) => {
      ...
      if (link_state !== "opted_back_in") {
        message = `${message}\n\n${time_message}`;
        this.__sendDMToUser(sender_object.id, receiver_object.screen_name.toLowerCase(), message, false, 'noop', 'reminder_menu');
      } else {
        redisClient.getAsync(redisHelpers.getQuizRunningKey(receiver_object.screen_name)).then((quiz_status) => {
          ...
          var menu_type = quiz_status == '1' ? false : 'reminder_menu';
