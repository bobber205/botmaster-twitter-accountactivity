...
redisClient.on('connect', function(err) {
  redisClient.getAsync(redisHelpers.getConfigurationKey()).then((config_json) => {
    ...
    console.log("CONFIG JSON IS", app_configuration.right_media_assets);
    console.log("CONFIG JSON IS", app_configuration.iq_media_assets[0]);
  });
...
class TwitterAccountActivityBot extends BaseBot {
    ...
    _.each(events, (direct_message_entry) => {
      ...
      join(quiz_def, extra_lives_def, iq_score_def, (whole_question_object, hash_values, score) => {
        ...
        if (whole_question_object) {
          ...
          if (question_matched && is_not_bot_entry) {
            ...
            direct_message_entry["answer_value"] = ['A','B','C','D'][_.findIndex(question_matched.all_answers, message_text_urls_removed)]
              ...
              join(responded_status_key, responded_status_key_by_answer_value).then((values) => {
                ...
                if (is_not_bot_entry) {
                    if (Math.random() > 0.9) {
                      ...
                      message_to_user = `${message_to_user} You can't win the prize, but you should keep playing to improve your IQ score, which is currently ${user_iq_score}. We'll post the results and next question on our feed in a couple of seconds!`;
                    } else if (responded_status_value != "1" && was_correct) {
           ...
           if (message_text_urls_removed.toUpperCase() == 'FAQ') {
             ...
             this.__sendUserIQScore(sender_id, sender_object.screen_name, user_iq_score);
           } else if (message_text_urls_removed.toUpperCase() == 'RANDOMTEST') {
             this.__sendDMToUser(sender_id, `some basic message`, false);
           }
           else if (is_not_bot_entry) {
              this.__sendDMToUser(sender_id, `Sorry, you answered too late! You can't win cash, but you should keep playing to improve your IQ score, which is currently ${user_iq_score}.`, true, `late`);
           } else {
             this.__standardBotMenuResponse(recipient_id, sender_id);
           }
          }
  ...
  __sendUserIQScore(sender_id, sender_handle, user_iq_score) {
    ...
    redisClient.hgetAsync(redisHelpers.getIQStatsKey(), sender_handle).then((result) => {
      if (!result) {
        this.__sendDMToUser(sender_id, `Your IQ Is not available until you have played your first game.`, false);
      } else {
        ...
        if (percentile == 0) {
          this.__sendDMToUser(sender_id, `Sorry but your IQ score is currently 0. Play more quizzes to increase your score!`, false);
        } else {
          this.__sendIQDMToUser(sender_id, `Your IQ score is ${percentile}\n${right_count} answers right and ${wrong_count} answers wrong`, user_iq_score);
        }
      }
    });
  }

  __sendIQDMToUser(recipient_id, message_text, user_iq_score) {
    ...
    var media_id_to_send = questionHelpers.getAppropriateMediaAssetForIQScore(app_configuration, (parseInt(user_iq_score) - 1));
