...
class TwitterAccountActivityBot {
      ...
      join(quiz_def, extra_lives_def, iq_score_def, user_bot_status_def, has_used_extra_life_for_quiz_def, has_extra_life_waiting_to_be_awarded_def, get_answer_status_key_def, get_answer_status_key_ttl_def, (whole_question_object, extra_life_redis_value, score, bot_state_status, has_used_extra_life_for_quiz, has_extra_life_waiting_to_be_awarded_value, get_answer_status_value, user_answer_ttl_value) => {
        ...
        if (whole_question_object) {
          ...
          if ( (answered_on_time || answered_late_but_its_ok) && question_matched) {
            ...
            if (!direct_message_entry.answer_value && message_text_urls_removed.toUpperCase() == "A.") {
              direct_message_entry.answer_value = 'A';
            }
            if (!direct_message_entry.answer_value && message_text_urls_removed.toUpperCase() == "B.") {
              direct_message_entry.answer_value = 'B';
            }
            if (!direct_message_entry.answer_value && message_text_urls_removed.toUpperCase() == "C.") {
              direct_message_entry.answer_value = 'C';
            }
            if (!direct_message_entry.answer_value && message_text_urls_removed.toUpperCase() == "D.") {
              direct_message_entry.answer_value = 'D';
            }
          ...
          //  else if (_.some(valid_opt_in_message, (current) => {return current.toLowerCase().indexOf(message_text_urls_removed.toLowerCase()) > -1 || message_text_urls_removed.toLowerCase().indexOf(current.toLowerCase()) > -1 })) {
           ...
           else if (message_text_urls_removed.toLowerCase() == '👎 i\'ve had enough' || is_not_bot_entry && message_text_urls_removed.toLowerCase() == 'i\'ve had enough') {
  ...
  __processUserHasOptedInEvent(sender_object, receiver_object, link_state = false) {
    ...
    join(get_stats_object_def, extra_lives_def, get_player_count_def).then((stats_object) => {
      ...
      if (link_state === "still_playing_reminder") {
        ...
        message = `
          Welcome @${sender_object.screen_name}, this is your ${thize(stats_object.quizzes_count + 1)} game! You're the ${thize(player_count)} in. Your IQ Score is ${stats_object.percentile} and you've got ${extra_life_count} extra lives.\n\nWe will be firing up the Questionator soon and you'll receive the first question here in your DMs!
        `;
  ...
  async __standardBotMenuResponse(recipient_id, sender_id, bot_owner_name, late_status = false) {
    ...
    console.log(`SIGH ${dm_payload.event.message_create.message_data.quick_reply.options.length}`);
  ...
  async __getStandardBotMenuOptionsObject(acting_handle_id = 'whocares') {
      ...
      if (true) {
        ...
        async function checkToSeeIfQuizRunning() {
          const res = await redisClient.getAsync(redisHelpers.getQuizRunningKey());
