...
class TwitterAccountActivityBot {
  ...
  __processDirectMessageEvent(messages)  {
    ...
    _.each(events, (direct_message_entry) => {
      ...
      var has_extra_life_waiting_to_be_awarded_def = redisClient.hgetAsync(redisHelpers.getWaitingToReceiveExtraLifeKey(`${receiver_object.screen_name.toLowerCase()}`), `${sender_object.screen_name.toLowerCase()}`);
      ...
      join(quiz_def, extra_lives_def, iq_score_def, user_bot_status_def, has_used_extra_life_for_quiz_def, has_extra_life_waiting_to_be_awarded_def, (whole_question_object, extra_life_redis_value, score, bot_state_status, has_used_extra_life_for_quiz, has_extra_life_waiting_to_be_awarded_value) => {
        console.log(`extra_life_redis_value ==> ${extra_life_redis_value}`);
        ...
        if (whole_question_object) {
          ...
          if (question_matched && has_extra_life_waiting_to_be_awarded_value) {
            people_to_award_to = _.split(has_extra_life_waiting_to_be_awarded_value, '@@@');
            _.each(people_to_award_to, (handle) => {
              redisClient.incrby(redisHelpers.getExtraLifeHashKey(receiver_object.screen_name.toLowerCase()), handle.toLowerCase(), 1, redis.print);
              var payload = {
                created_at: moment().utc().format(),
                acting_account: "",
                acting_account_id: "",
                receiving_account: handle,
          ...
          if (answered_on_time || answered_late_but_its_ok) {
            ...
            if (!was_correct && extra_life_status >= 1) {
              ...
              if (extra_life_status != 0) {
                redisClient.incrby(redisHelpers.getExtraLifeHashKey(receiver_object.screen_name.toLowerCase()), sender_object.screen_name.toLowerCase(), -1, redis.print);
              }
            ...
            if (reply_answer_status_to_user) {
              ...
              join(responded_status_key, responded_status_key_by_answer_value).then((values) => {
                ...
                if (is_not_bot_entry) {
                    if (Math.random() >= 1.70 && !extra_life_used) {
           ...
           if (message_text_urls_removed.toUpperCase() == 'FAQ' || message_text_urls_removed.toUpperCase() == 'SHOW ME SOME FAQS') {
             ...
             var right_before_game_link = "https://twitter.com/intent/tweet?url=pic.twitter.com/WxrK5fQyLb&text=Come%20play%20%40IQtrivia%20with%20me%20and%20win%20%F0%9F%92%B0.%20The%20game%20is%20starting%20right%20now%21%20%23Trivia%20%23IQtrivia";
           ...
           } else if (message_text_urls_removed.toUpperCase() === 'GET AN EXTRA LIFE (NEW)') {
             var redemption_code = sender_object.screen_name;
             var code_link = `https://twitter.com/intent/tweet?url=pic.twitter.com/ko6BlvPiCb&text=Come%20play%20%40IQtrivia%20with%20me%21%20Use%20my%20code%20%27${redemption_code.toUpperCase()}%27%20and%20you%27ll%20get%20an%20extra%20life.%20%23Trivia%20%23IQtrivia`
             this.__sendDMToUser(sender_id, receiver_object.screen_name, `To get an extra life just tell your friends to come play and enter your code: '${redemption_code.toUpperCase()}'. When a friend redeems your invite, and answers their first question, you'll get an extra life. Your friend will also get an extra life too! You can get as many extra lives as you want, and you can use one per game.\n\nShare your code to get extra lives: ${code_link}`, false, '', false);
           } else if (message_text_urls_removed.toUpperCase() === 'REDEEM AN INVITE FOR AN EXTRA LIFE') {
           ...
           } else if (message_text_urls_removed.toUpperCase() === 'REMIND ME WHEN THE NEXT GAME STARTS') {
  ...
  __parseFreeLifeCode(sender_object, receiver_object, code_submitted) {
    var is_valid_code_def = redisClient.hgetAsync(redisHelpers.getMasterUserList(receiver_object.screen_name), sender_object.screen_name.toLowerCase());
    ...
    join(is_valid_code_def, has_redeemed_code_already_def, (is_valid_code_value, has_redeemed_code_already_value) => {
      ...
      if (is_valid_code_value == "1" && (has_redeemed_code_already_value == "0" || !has_redeemed_code_already_value)) {
        this.__sendDMToUser(sender_id, receiver_object.screen_name, `Thanks! We'll give you and your friend an extra life when you answer your first question.`, false, '', true);
        var value = `${sender_object.screen_name.toLowerCase()}@@@${code_submitted}` //@ can't be in twitter usernames
        redisClient.hset(redisHelpers.getWaitingToReceiveExtraLifeKey(sender_object.screen_name), sender_object.screen_name().toLowerCase(), value);
        ...
        this.__sendDMToUser(sender_id, receiver_object.screen_name, `Sorry, it looks like that’s an invalid code. Tap below to try again.`, false, '', true);
        redisClient.set(redisHelpers.getUserBotStateKey(sender_object.screen_name), 'default', 'EX', 86400);
      }
    });
  }
  ...
  __sendUserIQScore(sender_id, sender_handle, bot_owner_name, user_iq_score) {
    ...
    redisClient.hgetAsync(redisHelpers.getIQStatsKey(), sender_handle.toLowerCase()).then((result) => {
      if (!result) {
        this.__sendDMToUser(sender_id, bot_owner_name, `Your IQ score isn't available until you've played your first game.\n\nCome play weekdays at 12:00 PDT!`, false);
      } else {
  ...
  __getStandardBotMenuOptionsObject(acting_handle_id = 'whocares') {
      ...
      if (['15588742', '19081905', '1375079370'].includes(acting_handle_id)) {
        var new_extra_life_menu = {
          label: "Get an Extra Life (NEW)",
          meta_data: "extra_life_new"
        };
        var redeem_code_menu = {
          label: "Redeem an invite for an extra life",
          meta_data: "redeem_code_menu"
        };
        options.splice(1, 0, new_extra_life_menu);
        options.splice(1, 0, redeem_code_menu);
      }
      return options;
  }
