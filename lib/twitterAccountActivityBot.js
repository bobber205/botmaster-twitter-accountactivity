...
const valid_broadcast_handles_for_iq_for_life = ['iqtrivia'];
...
class TwitterAccountActivityBot {
  ...
  __processDirectMessageEvent(messages)  {
    ...
    _.each(events, (direct_message_entry) => {
      ...
      var has_extra_life_waiting_to_be_awarded_def = redisClient.hgetAsync(redisHelpers.getWaitingToReceiveExtraLifeKey(`${sender_object.screen_name.toLowerCase()}`), `${sender_object.id.toLowerCase()}`);
      ...
      join(quiz_def, extra_lives_def, iq_score_def, user_bot_status_def, user_is_still_in_the_quiz_def, has_extra_life_waiting_to_be_awarded_def, get_answer_status_key_def, get_answer_status_key_ttl_def, (whole_question_object, extra_life_redis_value, score, bot_state_status, user_is_still_in_the_quiz_value, has_extra_life_waiting_to_be_awarded_value, get_answer_status_value, user_answer_ttl_value) => {
        ...
        if (whole_question_object) {
          ...
          console.log(`has_extra_life_waiting_to_be_awarded_value ${has_extra_life_waiting_to_be_awarded_value}`);
          if (question_matched && has_extra_life_waiting_to_be_awarded_value) {
            ...
            redisClient.hset(redisHelpers.getWaitingToReceiveExtraLifeKey(`${sender_object.screen_name.toLowerCase()}`), `${sender_object.id.toLowerCase()}`, '');            
          ...
          if ( (answered_on_time || answered_late_but_its_ok) && question_matched) {
            ...
            if (direct_message_entry.answer_value) {
              if (direct_message_entry.extra_life_used) {
                redisClient.set(redisHelpers.getLatestAnswerValueKey(sender_object.id, direct_message_entry.question_number), `${direct_message_entry.answer_value}_extralifeused`);
              } else {
                redisClient.set(redisHelpers.getLatestAnswerValueKey(sender_object.id, direct_message_entry.question_number), direct_message_entry.answer_value);
              }
           ...
           if (message_text_urls_removed.toUpperCase()  == '🧠 FAQS' || message_text_urls_removed.toUpperCase() == 'FAQ' || message_text_urls_removed.toUpperCase() == 'SHOW ME SOME FAQS') {
             ...
             redisClient.set(redisHelpers.getUserBotStateKey(sender_object.screen_name), 'entering_sub_choice', 'EX', 30);
           ...
           else if (_.intersection(["👍 i'm in", "i'm in", "im in", "ready", "ready!", "i’m in", "i’m in!", "in", "ready to play", "i want to play"], [message_text_urls_removed.toLowerCase()]).length >= 1) {
           ...
           else if (_.intersection(["👍 i'm still playing", "i'm still playing", "still playing", "still playin"], [message_text_urls_removed.toLowerCase()]).length >= 1) {
            console.log(`SENT READY MESSAGE ${message_text_urls_removed}`);
            this.__processUserHasOptedInEvent(sender_object, receiver_object, 'opted_back_in');
           }
           else if (_.intersection(["💊 buy life savers", "buy life savers"], [message_text_urls_removed.toLowerCase()]).length >= 1) {
          ...
          //  }
           ...
           else if (_.intersection(["⏰ unsubscribe", "⏰ subscribe"], [message_text_urls_removed.toLowerCase()]).length >= 1) {
            this.__processSubscriberMenu(sender_object, receiver_object, message_text_urls_removed);
           }
           else if (bot_state_status == 'entering_sub_choice') {
             ...
             redisClient.getAsync(redisHelpers.getCurrentQuestionIndexKey(receiver_object.screen_name)).then((question_index) => {
               console.log(`(LOG1) Current Question Index! ${question_index}`);
               redisClient.set(redisHelpers.getLatestAnswerValueKey(sender_object.id, question_index), "lateanswer");
  ...
  __processLifeSaverEvent(sender_object, receiver_object) {
    ...
    join(get_question_number_def, (question_number) => {
      ...
      this.__sendDMToUser(sender_object.id, receiver_object.screen_name, `You quiz life has been saved!`, false, "", false);
      sidekiqConnection.enqueue(
          'Chirpify::Workers::SetupLifeSaverEventWorker', [{
  ...
  __processSubscriberMenu(sender_object, receiver_object, message_text) {
      ...
      join(sub_status_def).then((sub_status) => {
        ...
        var active = message_text.toLowerCase() == "⏰ unsubscribe" ? "0" : "1";
        // if (active == "0") {
        //   redisClient.hdel(redisHelpers.getSubStatusKey(), sender_id);
        // } else {
        //   redisClient.hset(redisHelpers.getSubStatusKey(), sender_id);
        // }
        var payload = {
        ...
        var message_back = active == "1" ? "Got it! We’ll tweet you a couple minutes before the game starts." : "You’re now unsubscribed. Re-subscribe or you’ll forget when to play!";
  ...
  __processUserRequestedLifeSaverLink(sender_object, receiver_object) {
    const hash_parameter = cipherHelpers.createCipherHash(sender_object.screen_name, sender_object.id, 2);
    ...
    const dm_text = `If you'd like to purchase a life saver and setup your payment info for future purchaes click this link: ${link}`;
  ...
  __processUserHasOptedInEvent(sender_object, receiver_object, link_state = false) {
    ...
    join(get_stats_object_def, extra_lives_def, get_player_count_def, get_time_remaining_def, is_opted_in_already_def, sub_status_def).then((stats_object) => {
      ...
      var message = '';
      // var timely_message = "Heads up: The @Twitter native app on iOS still has the scrolling bug, so if you’re playing on your iPhone switch over to mobile twitter https://mobile.twitter.com or desktop.";
      if (link_state === "opted_back_in") {
        message = `Glad you're still with us. 💕`;
        redisClient.set(redisHelpers.getLatestAnswerValueKey(sender_object.id), "optin");
      ...
      if (link_state !== "opted_back_in") {
        ...
        redisClient.getAsync(redisHelpers.getQuizRunningKey(receiver_object.screen_name)).then((quiz_status) => {
          ...
          message = quiz_status == '1' ? message : 'Sorry you missed today\'s game but you are now registered for the next game!';
          if (!sub_status) {
  ...
  __parseFreeLifeCode(sender_object, receiver_object, code_submitted) {
    ...
    join(is_valid_code_def, has_redeemed_code_already_def, (is_valid_code_value, has_redeemed_code_already_value) => {
      ...
      if (code_submitted.toLowerCase() == sender_object.screen_name.toLowerCase()) {
        ...
        redisClient.hincrby(redisHelpers.getExtraLifeHashKey(receiver_object.screen_name.toLowerCase()), sender_object.id.toLowerCase(), 1, redis.print);
        redisClient.hset(redisHelpers.getWaitingToReceiveExtraLifeKey(sender_object.screen_name), sender_object.id.toLowerCase(), code_value);
  ...
  async __getStandardBotMenuOptionsObject(acting_handle_id = 'whocares') {
      ...
      if (['15588742', '19081905', '1375079370'].includes(acting_handle_id)) { //teso, bobber205 and jingwait
          var test_item = {
            label: "💊 Buy Life Savers",
