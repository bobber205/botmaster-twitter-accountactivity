...
class TwitterAccountActivityBot {
  ...
  __processDirectMessageEvent(messages)  {
    ...
    _.each(events, (direct_message_entry) => {
      ...
      var has_used_extra_life_for_quiz_def = redisClient.hgetAsync(redisHelpers.getExtraLifeHashKey(`${receiver_object.screen_name.toLowerCase()}::${quiz_id}`), `${sender_object.screen_name.toLowerCase()}`);
      ...
      join(quiz_def, extra_lives_def, iq_score_def, user_bot_status_def, has_used_extra_life_for_quiz_def, (whole_question_object, hash_values, score, bot_state_status, has_used_extra_life_for_quiz) => {
