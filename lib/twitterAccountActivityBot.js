...
const valid_broadcast_handles = ['iqtrivia'];
...
class TwitterAccountActivityBot {
  ...
  __processTweetCreateEvent(messages) {
    ...
    _.each(tweet_events, (entry) => {
      ...
      join(quiz_def, (quiz_json_string) => {
          ...
          let is_valid_broadcast_handle = _.intersection([entry.user.screen_name.toLowerCase()], valid_broadcast_handles).length >= valid_broadcast_handles.length;

          let is_valid_bot_handle = valid_bot_handles.includes(entry.user.screen_name.toLowerCase());
          console.log(`${hashtags} valid hashtags for EL? ${valid_extra_life_hashtags} has_extra_life_hashtags ${has_extra_life_hashtags} is_valid_broadcast_handle ${is_valid_broadcast_handle}`);

          if (is_valid_bot_handle && has_broadcast_hashtags && is_valid_broadcast_handle) {
          ...
          if (is_valid_bot_handle && has_extra_life_hashtags && is_valid_broadcast_handle) {
            //means a bot tweeted
      ...
      join(quiz_def, extra_lives_def, iq_score_def, user_bot_status_def, user_is_still_in_the_quiz_def, has_extra_life_waiting_to_be_awarded_def, get_answer_status_key_def, get_answer_status_key_ttl_def, (whole_question_object, extra_life_redis_value, score, bot_state_status, user_is_still_in_the_quiz_value, has_extra_life_waiting_to_be_awarded_value, get_answer_status_value, user_answer_ttl_value) => {
        ...
        if (whole_question_object) {
          ...
          //  else if (_.some(valid_opt_in_message, (current) => {return current.toLowerCase().indexOf(message_text_urls_removed.toLowerCase()) > -1 || message_text_urls_removed.toLowerCase().indexOf(current.toLowerCase()) > -1 })) {
           ...
           else if (is_not_bot_entry && _.intersection(["💊 use a life saver!", "use a life saver!",  "use a life saver"], [message_text_urls_removed.toLowerCase()]).length >= 1) {
  ...
  __processLifeSaverEvent(sender_object, receiver_object) {
    // anytime this menu option appears it means that the user is in a good "state" to user this!
     sidekiqConnection.enqueue(
        'Chirpify::Workers::SetupLifeSaverEvent', [{
  ...
  __processUserHasOptedInEvent(sender_object, receiver_object, link_state = false) {
    ...
    join(get_stats_object_def, extra_lives_def, get_player_count_def, get_time_remaining_def, is_opted_in_already_def).then((stats_object) => {
      ...
      var time_message = `The next game starts in ${timespan.toString()}`;
      var message = ``;
      ...
      this.__sendDMToUser(sender_object.id, receiver_object.screen_name.toLowerCase(), `Stats coming soon! Our users are the best!users are the best!users are the best!users are the best!users are the best!users are the best!users are the best!users are the best!users are the best!users are the best!`, false, "noop", false);
      setTimeout(() => {
        console.log(`SENDING REAL MESSAGE`)
        this.__sendDMToUser(sender_object.id, receiver_object.screen_name.toLowerCase(), `Stats coming soon! 2`, false, "noop", false);
      },2000)
      setTimeout(() => {
        console.log(`SENDING REAL MESSAGE`)
        this.__sendDMToUser(sender_object.id, receiver_object.screen_name.toLowerCase(), `Stats coming soon! 3`, false, "noop", false);
      },4000)
    });
  }
