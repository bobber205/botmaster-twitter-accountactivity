...
class TwitterAccountActivityBot {
  ...
  __sendUserIQScore(sender_id, sender_handle, bot_owner_name, user_iq_score) {
    ...
    join(get_stats_object_def, extra_lives_def).then((stats_object, extra_lives_count) => {
      ...
      if (!stats_object) {
        ...
        if (extra_lives_count == "0") {extra_life_prompt = "extra life";}
        if (percentile == 0) {
