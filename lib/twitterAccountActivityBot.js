...
class TwitterAccountActivityBot extends BaseBot {
  ...
  __processDirectMessageEvent(messages, request, response)  {
    ...
    _.each(events, (direct_message_entry) => {
      ...
      join(quiz_def, extra_lives_def, iq_score_def, (whole_question_object, extra_life_status, iq_score_object) => {
        console.log("EXTRA_LIVES_HASH", extra_life_status, iq_score_object);
        console.log(`iq score`, iq_score_object);
        let user_iq_score = iq_score_object['percentile'];
        ...
        if (whole_question_object) {
          ...
          if (question_matched && is_valid_answer) {
              ...
              join(responded_status_key, responded_status_key_by_answer_value).then((values) => {
                ...
                if (is_valid_answer) {
                    if (Math.random() > 0.8) {
                      ...
                      message_to_user = 'Sorry but ya gotta wait.';
                    }
           ...
           } else if (message_text_urls_removed == 'IQ Score' || message_text_urls_removed == 'IQ') {
             this.__sendUserIQScore(sender_id, sender_object.screen_name);
           }
           else if (is_valid_answer) {
              this.__sendDMToUser(sender_id, `Sorry but your answer was not send in time.`, true, 'late');
           } else {
             this.__standardBotMenuResponse(recipient_id, sender_id);
           }
          }
