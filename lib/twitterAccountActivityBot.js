  ...
  async __processChirpifyBotMessageEvent(users_array, events) {
    _.each(events, async (direct_message_entry) => {
      ...
      if (message_text.toLowerCase().includes('paypal')) {
        ...
        this.__sendDMToUser(sender_id, receiver_object.screen_name, `Log in and connect your PayPal account ${raw_link} ${!!initiated_tweet_id ? initiated_tweet_id : ''}`, false, '', false);
