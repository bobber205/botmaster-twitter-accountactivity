    ...
    _.each(tweet_events, (entry) => {
      ...
      join(quiz_def, (quiz_json_string) => {
        ...
        if (is_response_to_question) {
          console.log(`User ${entry.screen_name} is trying to respond to a question tweet!`);
          let to_message_user_id = entry.user.id_str;
          let to_message_screen_name = entry.screen_name;
          let message_to_send = `@${to_message_screen_name} Be sure to answer future questions using the "Send a a private message" button!`;
