    ...
    _.each(events, (direct_message_entry) => {
      ...
      join(quiz_def, extra_lives_def, iq_score_def, user_bot_status_def, has_used_extra_life_for_quiz_def, has_extra_life_waiting_to_be_awarded_def, get_answer_status_key_def, get_answer_status_key_ttl_def, (whole_question_object, extra_life_redis_value, score, bot_state_status, has_used_extra_life_for_quiz, has_extra_life_waiting_to_be_awarded_value, get_answer_status_value, user_answer_ttl_value) => {
        ...
        if (whole_question_object) {
          ...
          let looks_like_menu_answer = message_text_urls_removed.includes("A. ") || message_text_urls_removed.includes("B. ") || message_text_urls_removed.includes("C. ") || message_text_urls_removed.includes("D. ");
          let is_abcd = message_text_urls_removed.toUpperCase() == 'A' || message_text_urls_removed.toUpperCase() == 'B' || message_text_urls_removed.toUpperCase() == 'C' || message_text_urls_removed.toUpperCase() == 'D';
          ...
          //  } else if (looks_like_menu_answer) {
          //    this.__standardBotMenuResponse(recipient_id, sender_id, receiver_object.screen_name, true);
           }
           else if (!user_was_warned_about_type) {
             this.__standardBotMenuResponse(recipient_id, sender_id, receiver_object.screen_name);
           }
