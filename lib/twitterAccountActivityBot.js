...
class TwitterAccountActivityBot {
  ...
  __parseUserNumber(sender_object, receiver_object, phone_number_submitted) {
    ...
    if (is_valid) {
        ...
        sidekiqConnection.enqueue(
          'Chirpify::Workers::ProcessSMSSubEvent', [{sender_object: sender_object, phone_number: phoneUtil.format(number, PNF.INTERNATIONAL), raw_phone_number: number.getRawInput()}], {
            queue: 'default'
          }
        );
