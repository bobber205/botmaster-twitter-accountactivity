...
const valid_bot_handles = ['iqtrivia', 'sadtrebek'];
...
class TwitterAccountActivityBot {
  ...
  __processTweetCreateEvent(messages) {
    ...
    _.each(tweet_events, (entry) => {
      ...
      if (is_valid_bot_handle && has_broadcast_hashtags) {
        let dm_text = entry.text.replace(valid_broadcast_hashtags[0].toUpperCase(), '');
        dm_text = dm_text.replace('#', '');
        if (entry.extended_tweet) {
          dm_text = entry.extended_tweet.full_text.replace(valid_broadcast_hashtags[0].toUpperCase(), '');
          dm_text = dm_text.replace('#', '');
        }
  ...
  __processDirectMessageEvent(messages)  {
    ...
    _.each(events, (direct_message_entry) => {
      ...
      var can_use_extra_life_def = redisClient.hgetAsync(redisHelpers.getEligibleForExtraLifeUsageKey(`${receiver_object.screen_name.toLowerCase()}`), `${sender_object.id}`);
      ...
      join(quiz_def, extra_lives_def, iq_score_def, user_bot_status_def, can_use_extra_life_def, has_extra_life_waiting_to_be_awarded_def, get_answer_status_key_def, get_answer_status_key_ttl_def, (whole_question_object, extra_life_redis_value, score, bot_state_status, can_use_extra_life_on_this_quiz, has_extra_life_waiting_to_be_awarded_value, get_answer_status_value, user_answer_ttl_value) => {

        let extra_life_status = parseInt(extra_life_redis_value || `0`);
        let iq_score_object = JSON.parse(score || '{}');

        console.log(`can_use_extra_life_on_this_quiz ===> ${can_use_extra_life_on_this_quiz}`);
        can_use_extra_life_on_this_quiz = can_use_extra_life_on_this_quiz || 0;


        can_use_extra_life_on_this_quiz = can_use_extra_life_on_this_quiz == "1" ? true : false; //translate to a boolean
        console.log(`can_use_extra_life_on_this_quiz ==> ${can_use_extra_life_on_this_quiz}`);

        console.log(`TTL IS ${user_answer_ttl_value}`);

        if (!can_use_extra_life_on_this_quiz) {
          extra_life_status = "0";
        }

        let answered_via_dm = false;
        if (!welcome_message_id_found) {
          console.log(`get_answer_status_value Not Welcome Message ID Found in Payload, using ${get_answer_status_value} instead ${redisHelpers.getAnswerStatusKey(sender_object.id)}`);
        ...
        let is_not_bot_entry = _.intersection([message_text_urls_removed.trim().toLowerCase()], ['i\'ve had enough', 'iq score', 'faq', 'get an extra life', '#protips', 'get an extra life', 'tell me some #protips', 'submit feedback', 'get my iq score and game stats', 'get your iq score and game stats', '💰 Paypal', '📧 Contact', '🧠 FAQs', '⏰ Reminders', '😎 Extra Lives', '🤟 Redeem', '📈 Stats']).length == 0;
        ...
        if (whole_question_object) {
          ...
          if (is_last_question) {
            console.log(`${sender_object.screen_name} is on the last question!`);
            extra_life_status = 0;
          }
          ...
          console.log(`${sender_object.screen_name} TTL ==> question_matched == ${question_matched}`);
          ...
          console.log(`answered_late_but_its_ok TTL  ==> ${answered_late_but_its_ok}`);
          ...
          if ( (answered_on_time || answered_late_but_its_ok) && question_matched) {
            ...
            // console.log(`Deleting instead ---> ${redisHelpers.getAnswerStatusKey(sender_object.id)}`);
            ...
            if (direct_message_entry.answer_value) {
              if (direct_message_entry.extra_life_used) {
                redisClient.set(redisHelpers.getLatestAnswerValueKey(sender_object.id), `${direct_message_entry.answer_value}_extralifeused`);
              } else {
                redisClient.set(redisHelpers.getLatestAnswerValueKey(sender_object.id), direct_message_entry.answer_value);
              }
           ...
           else if (_.intersection(["👍 i'm in", "i'm in", "ready", "i’m in", "in", "ready to play"], [message_text_urls_removed.toLowerCase()]).length >= 1) {
          //  else if (_.some(valid_opt_in_message, (current) => {return current.toLowerCase().indexOf(message_text_urls_removed.toLowerCase()) > -1 || message_text_urls_removed.toLowerCase().indexOf(current.toLowerCase()) > -1 })) {
           ...
           else if (_.intersection(["👍 i want in"], [message_text_urls_removed.toLowerCase()]).length >= 1) {
          //  else if (_.some(valid_opt_in_message, (current) => {return current.toLowerCase().indexOf(message_text_urls_removed.toLowerCase()) > -1 || message_text_urls_removed.toLowerCase().indexOf(current.toLowerCase()) > -1 })) {
            console.log(`SENT READY MESSAGE ${message_text_urls_removed}`);
            this.__processUserHasOptedInEvent(sender_object, receiver_object, 'public_tweet_link');
           }
           ...
           else if (_.intersection(["👍 i'm still playing"], [message_text_urls_removed.toLowerCase()]).length >= 1) {
          //  else if (_.some(valid_opt_in_message, (current) => {return current.toLowerCase().indexOf(message_text_urls_removed.toLowerCase()) > -1 || message_text_urls_removed.toLowerCase().indexOf(current.toLowerCase()) > -1 })) {
            console.log(`SENT STILL IN ${message_text_urls_removed}`);
            this.__processUserHasOptedInEvent(sender_object, receiver_object, 'still_playing_reminder');
           }
           else if (_.intersection(["👍 hell yeah"], [message_text_urls_removed.toLowerCase()]).length >= 1) {
          //  else if (_.some(valid_opt_in_message, (current) => {return current.toLowerCase().indexOf(message_text_urls_removed.toLowerCase()) > -1 || message_text_urls_removed.toLowerCase().indexOf(current.toLowerCase()) > -1 })) {
            console.log(`SENT STILL IN ${message_text_urls_removed}`);
            this.__processUserHasOptedInEvent(sender_object, receiver_object, "public_tweet_link");
           }
            ...
            this.__sendDMToUser(sender_id, receiver_object.screen_name, ` Ok, see you next game, I hope!\nYou can check your stats after the game is over.`, false, '', false);
           }
           else if (is_not_bot_entry && _.intersection(["👎 i'm out", '👎 nah'], [message_text_urls_removed.toLowerCase()]).length >= 1) {
            this.__sendDMToUser(sender_id, receiver_object.screen_name, `Ok, see you next game, I hope!`, false, '', false);
           }
           else if (false) {
             redisClient.hdel(redisHelpers.getPlayerListKey(receiver_object.screen_name), sender_object.id.toLowerCase(), "1");
             this.__sendDMToUser(sender_id, receiver_object.screen_name, `Ok, see you next game, I hope!`, false, "", false);
  ...
  __processUserHasOptedInEvent(sender_object, receiver_object, link_state = false) {
    ...
    join(get_stats_object_def, extra_lives_def, get_player_count_def, get_time_remaining_def, is_opted_in_already_def).then((stats_object) => {
      ...
      console.log(`OPT IN STATUS ${opted_in_status}`);
      ...
      redisClient.set(redisHelpers.getLatestAnswerValueKey(sender_object.id), 'z');
      
      var remaining_ms = (next_game_time*1000) - moment.utc();

      var days_remaining = Math.floor(remaining_ms / (60 * 60 * 1000) / 24);

      var hours_remaining = Math.floor(remaining_ms / (60 * 60 * 1000));
      if (hours_remaining >= 24) {
        hours_remaining = Math.floor((remaining_ms%(1000 * 60 * 60 * 24)) / (60 * 60 * 1000));
      }
      
      var minutes_remaining = Math.floor((remaining_ms % (1000 * 60 * 60)) / (1000 * 60));
      var seconds_remaining = Math.floor((remaining_ms % (1000 * 60)) / 1000);
      var time_message;
      var time_message_days = days_remaining > 1 ? 'days': `day`;
      var time_message_hours = hours_remaining > 1 ? 'hours': `hour`;
      var time_message_minutes = minutes_remaining > 1 ? 'minutes' : `minute`;
      var questinator_message = "The Questionator will send you the first question here in your DMs when the game begins!";

      console.log(`next_game_time ${next_game_time} days_remaining ${days_remaining} hours_remaining ${hours_remaining} minutes_remaining ${minutes_remaining} seconds_remaining ${seconds_remaining}`);

      if ((minutes_remaining < 1 && hours_remaining < 1) || minutes_remaining == NaN) {
        time_message = "The game is in progress. Next question coming right up!";
      }
      else if (hours_remaining >= 24) {
        time_message = `The next game starts in ${days_remaining} ${time_message_days} and ${hours_remaining} ${time_message_hours}.`
      }
      else if (hours_remaining < 1) {
        time_message = `The next game starts soon! In ${minutes_remaining} ${time_message_minutes} to be exact!`;
      } else {
        time_message = `The game starts in ${hours_remaining} ${time_message_hours} and ${minutes_remaining} ${time_message_minutes}.`;
      }
      time_message = `The next game is on Monday, September 24th 12:30 PDT! Checkout https://www.iqtrivia.live for a countdown.`
      var message = ``;
      if (link_state === "still_playing_reminder") {
        message = `Ok, next question coming right up!`;
        redisClient.set(redisHelpers.getLatestAnswerValueKey(sender_object.id), "z");
        ...
        if (opted_in_status == 1) {
          ...
          message = `
            Welcome @${sender_object.screen_name}, this is your ${thize(stats_object.quizzes_count + 1)} game! You're the ${thize(player_count)} player registered for this next game. Your IQ Score is ${stats_object.percentile} and you've got ${extra_life_count} extra lives.\n\n${questinator_message}`;
        }
      } else if (link_state === "public_tweet_link") {
          message = `Ok, next question coming right up!`;
          redisClient.set(redisHelpers.getLatestAnswerValueKey(sender_object.id), "z");
      }
      message = `${message}\n\n${time_message}`
