...
const valid_bot_handles = ['iqtrivia', 'sadtrebek', 'happytrebek', 'alextest_01'];
...
class TwitterAccountActivityBot {
  ...
  async __processChirpifyBotMessageEvent(users_array, events) {
    _.each(events, async (direct_message_entry) => {
      ...
      let chirpify_bot_config = chirpify_bot_handles[receiver_username];
      ...
      if (number && is_valid) {
        ...
        await axios_instance.get('/v2.0/users/populate').then((response) => {
        ...
        let register_result = await axios_instance.post('/v3.0/registration/via_bot', register_payload);
  ...
  __processDirectMessageEvent(messages) {
    ...
    _.each(events, (direct_message_entry) => {
      ...
      if (chirpify_bot_handles[receiver_object.screen_name]) {
        console.log("Processing as a Chirpify Bot", chirpify_bot_handles[receiver_object.screen_name])
