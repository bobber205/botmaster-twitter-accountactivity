...
class TwitterAccountActivityBot {
  ...
  __sendIQDMToUser(recipient_id, bot_owner_name, message_text, user_iq_score) {
    ...
    var media_id_to_send = questionHelpers.getAppropriateMediaAssetForIQScore(iq_score_configuration[bot_owner_name], (parseInt(user_iq_score) - 1));
  ...
  __sendDMToUser(recipient_id, bot_owner_name, message_text, send_image = true, image_type = 'correct') {
    ...
    if (send_image) {
      var media_id_to_send = questionHelpers.getAppropriateMediaAsset(app_configuration[bot_owner_name], image_type);
      dm_payload.event.message_create.message_data.attachment = {
        type: "media",
        media: {
          id: media_id_to_send
