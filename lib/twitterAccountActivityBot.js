  ...
  __processDirectMessageEvent(messages)  {
    ...
    _.each(events, (direct_message_entry) => {
      ...
      join(quiz_def, extra_lives_def, iq_score_def, user_bot_status_def, has_used_extra_life_for_quiz_def, has_extra_life_waiting_to_be_awarded_def, (whole_question_object, extra_life_redis_value, score, bot_state_status, has_used_extra_life_for_quiz, has_extra_life_waiting_to_be_awarded_value) => {
        ...
        let is_not_bot_entry = _.intersection([message_text_urls_removed.trim().toLowerCase()], ['iq score', 'faq', 'get an extra life', '#protips', 'get an extra life', 'tell me some #protips', 'submit feedback', 'get my iq score and game stats']).length == 0;
        ...
        if (whole_question_object) {
          ...
          if (answered_on_time || answered_late_but_its_ok) {
           ...
           } else if (message_text_urls_removed.toUpperCase() == 'IQ SCORE' || message_text_urls_removed.toUpperCase() == 'IQ' || message_text_urls_removed.toUpperCase() == 'GET MY IQ SCORE AND GAME STATS') {
  ...
  __parseFreeLifeCode(sender_object, receiver_object, code_submitted) {
    ...
    join(is_valid_code_def, has_redeemed_code_already_def, (is_valid_code_value, has_redeemed_code_already_value) => {
      ...
      if (code_submitted.toLowerCase() == sender_object.screen_name.toLowerCase()) {
        ...
        var redeemer_name = sender_object.screen_name;
        this.__sendDMToUser(is_valid_code_value, receiver_object.screen_name, `@${redeemer_name} just redeemed your code! You’ll get your free life when they answer their first question. Want more lives? Invite more friends!`, false, '', false);
