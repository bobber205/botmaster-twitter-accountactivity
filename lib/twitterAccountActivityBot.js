...
class TwitterAccountActivityBot {
  ...
  __processUserHasOptedInEvent(sender_object, receiver_object, link_state = false) {
    ...
    var life_saver_count_def = redisClient.getAsync(redisHelpers.getLifeSaverQuestionNumberKey(sender_object.id));
    ...
    join(get_stats_object_def, extra_lives_def, get_player_count_def, get_time_remaining_def, is_opted_in_already_def, sub_status_def, life_saver_count_def).then((stats_object) => {
      ...
      if (link_state === "opted_back_in") {
        ...
        if (opted_in_status == 1) {
          ...
          message = `
            Welcome @${sender_object.screen_name}, this is your ${thize(stats_object.quizzes_count + 1)} game! You're the ${thize(player_count)} player registered for this next game. Your IQ Score is ${stats_object.percentile} and you've got ${extra_life_count} extra lives along with ${life_saver_count} life savers purchased.\n\n${questinator_message}`;
        }
      } 
      if (link_state !== "opted_back_in") {
        message = `${message}\n\n${time_message}`;
  ...
  __sendUserIQScore(sender_id, sender_handle, bot_owner_name, user_iq_score) {
    ...
    var life_saver_count_def = redisClient.getAsync(redisHelpers.getLifeSaverQuestionNumberKey(sender_id));
