...
class TwitterAccountActivityBot {
  ...
  __processTweetCreateEvent(messages) {
    ...
    _.each(tweet_events, (entry) => {
      hashtags = entry.entities.hashtags.map((c)=> {return c.text.toLowerCase();});
      let has_right_hashtags = hashtags.includes('iqtrivia') && hashtags.includes('trivia');
      if (has_right_hashtags && entry.user.screen_name === 'bobber205') {
        var payload = {
          status: `@${entry.user.screen_name} Your Now Have an Extra Life for the Next Quiz!`,
