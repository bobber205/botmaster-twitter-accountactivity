...
class TwitterAccountActivityBot {
    ...
    _.each(tweet_events, (entry) => {
      ...
      if (is_valid_bot_handle && has_extra_life_hashtags) {
        ...
        _.each(users_mentioned, (user_data) => {
          ...
          redisClient.hincrby(redisHelpers.getExtraLifeHashKey(entry.user.screen_name.toLowerCase()), user_screen_name.toLowerCase(), 2, redis.print);
          this.__sendDMToUser(user_id, entry.user.screen_name.toLowerCase(), `You've been given 2 bonus extra lives!`, false, '', false);
        });
      }
    });
  }
  ...
  __processDirectMessageEvent(messages)  {
    ...
    _.each(events, (direct_message_entry) => {
      ...
      join(quiz_def, extra_lives_def, iq_score_def, user_bot_status_def, has_used_extra_life_for_quiz_def, has_extra_life_waiting_to_be_awarded_def, get_answer_status_key_def, get_answer_status_key_ttl_def, (whole_question_object, extra_life_redis_value, score, bot_state_status, has_used_extra_life_for_quiz, has_extra_life_waiting_to_be_awarded_value, get_answer_status_value, user_answer_ttl_value) => {
        ...
        if (whole_question_object) {
          ...
          //  else if (_.some(valid_opt_in_message, (current) => {return current.toLowerCase().indexOf(message_text_urls_removed.toLowerCase()) > -1 || message_text_urls_removed.toLowerCase().indexOf(current.toLowerCase()) > -1 })) {
            console.log(`SENT STILL IN ${message_text_urls_removed} ${valid_opt_in_message}`);
