  ...
  __processUserHasOptedInEvent(sender_object, receiver_object, link_state = false) {
    ...
    join(get_stats_object_def, extra_lives_def, get_player_count_def, get_time_remaining_def, is_opted_in_already_def).then((stats_object) => {
      ...
      if (link_state !== "opted_back_in") {
        ...
        redisClient.getAsync(redisHelpers.getQuizRunningKey(process.env.BOT_HANDLE)).then((quiz_status) => {
          console.log(`QUIZ STATUS ${quiz_status}`)
          var menu_type = redisClient.get(redisHelpers.getQuizRunningKey(process.env.BOT_HANDLE)) == '1' ? false : 'reminder_menu';
          message = redisClient.get(redisHelpers.getQuizRunningKey(process.env.BOT_HANDLE)) == '1' ? false : 'Sorry you missed today\s game! Do you want to be reminded for the next one? 👇;
