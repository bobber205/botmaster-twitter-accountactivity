...
class TwitterAccountActivityBot {
  ...
  __processDirectMessageEvent(messages)  {
    ...
    _.each(events, (direct_message_entry) => {
      ...
      join(quiz_def, extra_lives_def, iq_score_def, user_bot_status_def, (whole_question_object, hash_values, score, bot_state_status) => {
        console.log(`bot_state_status ${bot_state_status}`);
        ...
        if (whole_question_object) {
          ...
          if (answered_on_time || answered_late_but_its_ok) {
           ...
           if (message_text_urls_removed.toUpperCase() == 'FAQ' || message_text_urls_removed.toUpperCase() == 'SHOW ME SOME FAQS') {
             ...
             var link = "https://twitter.com/intent/tweet?url=https%3A%2F%2Fpic.twitter.com/oCaCam3Rhf&text=Come%20play%20@IQtrivia%20with%20me%21%20It%27s%20a%20trivia%20game%20played%20for%20cash%20money%20right%20here%20on%20Twitter.%20%23Trivia%20%23IQtrivia";
             ...
             this.__sendDMToUser(sender_id, receiver_object.screen_name, `Just reply with your full phone number and we’ll text you when the game is about to start.\n\nU.S. example: 503.555.5656\n\nNon U.S. example: +44 20 7234 3456 (include your country code)`, false, '', false);
           }
  ...
  __parseUserNumber(sender_object, receiver_object, phone_number_submitted) {
    ...
    var code_found = PhoneNumber(phone_number_submitted).getRegionCode();
    ...
    client.lookups.phoneNumbers(phone_number_submitted)
      ...
      .then((phone_number_object) => {
        ...
        if (code_found) {
          ...
          this.__sendDMToUser(sender_object.id, receiver_object.screen_name, `Thanks! We sent you a text message. If you didn’t receive it tap below to try again.`, false, '', 'done_with_phone_number_bot_menu');
        } else {
          this.__sendDMToUser(sender_object.id, receiver_object.screen_name, `No Country Code Was Found -- Please Try Again`, false, '', 'done_with_phone_number_bot_menu');
        }
      })
      .catch((e) => {
  ...
  __sendDMToUser(recipient_id, bot_owner_name, message_text, send_image = true, image_type = 'correct', bot_menu_type = false) {
    ...
    if (bot_menu_type === 'done_with_phone_number_bot_menu') {
      dm_payload.event.message_create.message_data.quick_reply = {
        type: 'options',
        options: this.__getPhoneNumberDoneMenu(recipient_id)
      };
    }
  ...
  __getPhoneNumberDoneMenu(acting_handle_id = 'whocares') {
    var options = [
      ...
      {
        label: "Notify me when the next game starts",
        meta_data: "Notify me when the next game starts"
      }
    ];
    return options;
  }
