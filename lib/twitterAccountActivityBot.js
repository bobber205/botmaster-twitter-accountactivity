...
class TwitterAccountActivityBot {
      ...
      join(quiz_def, extra_lives_def, iq_score_def, user_bot_status_def, has_used_extra_life_for_quiz_def, has_extra_life_waiting_to_be_awarded_def, (whole_question_object, extra_life_redis_value, score, bot_state_status, has_used_extra_life_for_quiz, has_extra_life_waiting_to_be_awarded_value) => {
        ...
        if (whole_question_object) {
          ...
          if (answered_on_time || answered_late_but_its_ok) {
           ...
           } else if (message_text_urls_removed.toUpperCase() === 'ADD MY PAYPAL EMAIL TO GET PAID') {
  ...
  __validatePaypalEmailAddress(sender_object, receiver_object, email_address) {
    ...
    if (is_email_address) {
      ...
      this.__sendDMToUser(sender_id, receiver_object.screen_name, `Sorry, that doesn't look like a valid email address. Try again or press the I'm done button`, false, "", "done_with_this_state");
      redisClient.set(redisHelpers.getUserBotStateKey(sender_object.screen_name), "entering_paypal_email", "EX", 60 * 1);
    }
  }
  ...
  __getStandardBotMenuOptionsObject(acting_handle_id = 'whocares') {
      ...
      if (['15588742', '19081905', '1375079370'].includes(acting_handle_id)) {
        var enter_your_email_item = {
          label: "Add my PayPal email to get paid",
          meta_data: "paypal_email_menu_item"
        };
        options.splice(1, 0, enter_your_email_item);
      }
      return options;
  }
