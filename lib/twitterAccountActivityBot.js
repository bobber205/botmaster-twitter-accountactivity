...
class TwitterAccountActivityBot extends BaseBot {
  ...
  __processDirectMessageEvent(messages, request, response)  {
    ...
    _.each(events, (direct_message_entry) => {
      ...
      join(quiz_def, extra_lives_def, iq_score_def, (whole_question_object, extra_life_status, iq_score) => {
        console.log("EXTRA_LIVES_HASH", extra_life_status);
        console.log(`iq score`, iq_score);
          ...
          if (question_matched) {
            ...
            if (reply_answer_status_to_user) {
              ...
              join(responded_status_key, responded_status_key_by_answer_value).then((values) => {
                ...
                var status_value = values[1];
                console.log("fucking values wtf", values);
                console.log('Status Value! ==> ', already_responded_value, status_value, arguments);
                if (is_valid_answer) {
                    if (Math.random() > 0.8) {
