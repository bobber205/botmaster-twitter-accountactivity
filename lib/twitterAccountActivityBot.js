      ...
      join(quiz_def, extra_lives_def, (whole_question_object, extra_life_status) => {
        ...
        if (whole_question_object) {
          ...
          if (question_matched) {
            ...
            let is_valid_answer = _.intersection([message_text_urls_removed.trim().toLowerCase()], ['a','b','c','d','test']).length > 0;
            ...
            if (reply_answer_status_to_user) {
              ...
              redisClient.hgetAsync(redisHelpers.getRespondedAnswerKey(receiver_object.screen_name, question_matched.question_number), sender_object.screen_name).then((already_responded_value) => {
                ...
                if (already_responded_value != "1" && is_valid_answer) {
                  ...
                  this.__sendDMToUser(sender_id, `Sorry ${sender_object.screen_name} but the rest of the bot isn't active yet`, false);
                }
