...
class TwitterAccountActivityBot {
  ...
  __processDirectMessageEvent(messages)  {
    ...
    _.each(events, (direct_message_entry) => {
      ...
      join(quiz_def, extra_lives_def, iq_score_def, user_bot_status_def, has_used_extra_life_for_quiz_def, (whole_question_object, extra_life_redis_value, score, bot_state_status, has_used_extra_life_for_quiz) => {
        ...
        if (whole_question_object) {
          ...
          if (answered_on_time || answered_late_but_its_ok) {
           ...
           if (message_text_urls_removed.toUpperCase() == 'FAQ' || message_text_urls_removed.toUpperCase() == 'SHOW ME SOME FAQS') {
             ...
             redisClient.set(redisHelpers.getUserBotStateKey(sender_object.screen_name), 'entering_phone_number', 'EX', 60 * 2);
