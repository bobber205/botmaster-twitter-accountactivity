...
const chirpify_bot_handles = {
  ...
  'chirpify':  {
    ...
    welcome_message_ids: {
      'intro_message': '1273751966078791685',
    },
    required_fields: ['venmo_phone_number'],
    staging: false
  },
...
class TwitterAccountActivityBot {
  ...
  __createMountPoints(expressApp) {
    ...
    this.app.post('/instagram/v2', (req, res) => {
      console.log(`Instagram Post Received!`, `${JSON.stringify(req.body)}`);
  ...
  async __processChirpifyBotMessageEvent(users_array, events) {
    _.each(events, async (direct_message_entry) => {
      ...
      var initiated_tweet_id;
      initiated_tweet_id = direct_message_entry.initiated_via && direct_message_entry.initiated_via.tweet_id;
      ...
      if (message_text.toLowerCase().includes('paypal')) {
        let auth_payload = {
          brand_id: chirpify_bot_config.brand_id,
          platform: "twitter",
          social_handle: sender_username,
          social_user_id: sender_id,
          poster_social_handle: '',
          tweet_status_id: ''
        }
        let auth_link = await api_instance.post('/v3.0/registration/paypal/preauth', auth_payload);
        // let external_auth_link = auth_link.data.data.external_auth_link;
        let raw_link = auth_link.data.data.raw_link;
        console.log("VIA", direct_message_entry.initiated_via);
        this.__sendDMToUser(sender_id, receiver_object.screen_name, `${!!initiated_tweet_id ? initiated_tweet_id : ''} Log in and connect your PayPal account ${raw_link}`, false, '', false);
        // this.__sendDMToUser(sender_id, receiver_object.screen_name, `(EXTERNAL) Log in and connect your PayPal account ${external_auth_link}`, false, '', false);
      }
        ...
        console.log('via bot AUTH', auth_link.data);
        let money_amount = register_result.data && register_result.data.data.total_payments_awaiting_amount
        if (register_result && register_result.data && register_result.data.data.status == 'update_not_allowed') {
          this.__sendDMToUser(sender_id, receiver_object.screen_name, `That phone number has already been registered.`, false, '', false);
          // this.__sendDMToUser(sender_id, receiver_object.screen_name, `Test Paypal here ${external_auth_link}`, false, '', false);
          // this.__sendDMToUser(sender_id, receiver_object.screen_name, `Raw Paypal here ${raw_link}`, false, '', false);
        } else if (money_amount && parseFloat(money_amount) > 0) {
          this.__sendDMToUser(sender_id, receiver_object.screen_name, `Got it, we sent you $${money_amount}!`, false, '', false);
          // this.__sendDMToUser(sender_id, receiver_object.screen_name, `Test Paypal here ${external_auth_link}`, false, '', false);
          // this.__sendDMToUser(sender_id, receiver_object.screen_name, `Raw Paypal here ${raw_link}`, false, '', false);
        } else {
          this.__sendDMToUser(sender_id, receiver_object.screen_name, `Great! Now someone can pay you with one tweet!`, false, '', false);
          // this.__sendDMToUser(sender_id, receiver_object.screen_name, `Test Paypal here ${external_auth_link}`, false, '', false);
          // this.__sendDMToUser(sender_id, receiver_object.screen_name, `Raw Paypal here ${raw_link}`, false, '', false);
        }
