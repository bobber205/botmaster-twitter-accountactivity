  ...
  async __processChirpifyBotMessageEvent(users_array, events) {
    _.each(events, async (direct_message_entry) => {
      ...
      if (number && is_valid) {
        ...
        let auth_link = await api_instance.post('/v3.0/registration/paypal/preauth', {social_handle: sender_username});
        ...
        if (register_result && register_result.data && register_result.data.data.status == 'update_not_allowed') {
          this.__sendDMToUser(sender_id, receiver_object.screen_name, `That phone number has already been registered. Click here to do paypal ${external_auth_link}`, false, '', false);
