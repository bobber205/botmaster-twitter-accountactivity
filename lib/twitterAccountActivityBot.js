...
class TwitterAccountActivityBot {
    ...
    _.each(events, (direct_message_entry) => {
      ...
      join(quiz_def, extra_lives_def, iq_score_def, (whole_question_object, hash_values, score) => {
        ...
        if (dm_url) {
          ...
          console.log(`No URL to remove`);
          message_text_urls_removed = original_message_text;
        }

        let is_not_bot_entry = _.intersection([message_text_urls_removed.trim().toLowerCase()], ['iq score', 'faq', 'get an extra life', '#protips']).length == 0;
        ...
        if (whole_question_object) {
          ...
          let answered_late_but_its_ok = question_matched && is_not_bot_entry && question_matched['current'] == '0' && extra_life_status == "1";
          ...
          if (answered_on_time || answered_late_but_its_ok) {
            ...
            if (reply_answer_status_to_user) {
              ...
              join(responded_status_key, responded_status_key_by_answer_value).then((values) => {
                ...
                if (is_not_bot_entry) {
                    var link_message = `Tap our avatar to to go there.`
                    // if (Math.random())
                    if (Math.random() > 0.91) {
           ...
           if (message_text_urls_removed.toUpperCase() == 'FAQ') {
           ...
           } else if (message_text_urls_removed.toUpperCase() == '#PROTIPS') {
             this.__sendDMToUser(sender_id, receiver_object.screen_name, `You can use any Twitter owned application to play, however we’ve found the best way to play is using Twitter’s mobile app and turning on tweet notifications for our account. That way you’ll catch every question without having to manually refresh your feed. Tap here to see how to turn on notifications https://cdn-images-1.medium.com/max/800/1*mzIh1uTgmErcDz8bg0XtWg.gif`, false);
           } else if (message_text_urls_removed.toUpperCase() == 'GET AN EXTRA LIFE') {
             ...
             this.__sendDMToUser(sender_id, receiver_object.screen_name, `To get an extra life all you need to do is share the game by tapping here ${link}`, false);
           }
  ...
  __sendUserIQScore(sender_id, sender_handle, bot_owner_name, user_iq_score) {
    ...
    redisClient.hgetAsync(redisHelpers.getIQStatsKey(), sender_handle).then((result) => {
      if (!result) {
        ...
        // this.__standardBotMenuResponse('', sender_id, bot_owner_name);
      }
    });
  }
  ...
  __sendIQDMToUser(recipient_id, bot_owner_name, message_text, user_iq_score) {
    ...
    console.log(bot_owner_name)
    console.log(iq_score_configuration)
    ...
    dm_payload.event.message_create.message_data.quick_reply = {
      type: 'options',
      options: this.__getStandardBotMenuOptionsObject()
    }
  ...
  __logErrorEvent(recipient_id, error_object) {
    ...
    if (!error_object["message"]) return;
    sidekiqConnection.enqueue(
  ...
  __getStandardBotMenuOptionsObject() {
      var options = [{
        label: 'FAQ',
  ...
  __processWebhookEvent(messages) {
    if (_.intersection[messages["for_user_id"]], ["136023366", "4749662233", "498326257", "415839284"]) return;
