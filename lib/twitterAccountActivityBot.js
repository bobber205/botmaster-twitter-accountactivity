...
class TwitterAccountActivityBot {
  ...
  __parseUserNumber(sender_object, receiver_object, phone_number_submitted) {
    ...
    client.lookups.phoneNumbers(phone_number_submitted)
      ...
      .then((phone_number) => {
        console.log(`phone_number.callerName: ${JSON.stringify(phone_number)}`);
        code_found = true;
        if (code_found) {
          ...
          sidekiqConnection.enqueue(
            'Chirpify::Workers::ProcessSMSSubEvent', [{
              ...
              phone_number: phoneUtil.format(number, PNF.INTERNATIONAL),
              raw_phone_number: phone_number_submitted
            }], {
              queue: 'default'
            }
          );
          this.__sendDMToUser(sender_object.id, receiver_object.screen_name, `${message} ==> ${phone_output}`, false, '', 'done_with_phone_number_bot_menu');
        } else {
