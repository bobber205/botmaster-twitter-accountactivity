...
module.exports.getWaitingToReceiveExtraLifeKey = (handle) => {
  return `Chirpify::IQTrivia::${handle.toLowerCase()}::WaitingToReceiveExtraLifeV2`;
};
...
module.exports.getUserBotStateKey = (handle, bot_handle) => {
  return `Chirpify::Configuration::${bot_handle.toLowerCase()}::${handle.toLowerCase()}::BotStateKey`;
};
...
module.exports.getSubStatusKey = () => {
  var handle = 'iqtrivia';
  return `Chirpify::IQTrivia::${handle.toLowerCase()}::SubbedToReminders`;
};
...
module.exports.getLifeSaverCountKey = (twitter_account_id) => {
  return `Chirpify::IQTrivia::CurrentLifeSaverCounter::${twitter_account_id}`;
};