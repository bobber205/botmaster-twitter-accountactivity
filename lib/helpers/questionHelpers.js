...
module.exports.determineQuestionAnsweredByWelcomeID = (welcome_id, master_question_object) => {
  console.log(`Searching Welcome ID ==> ${welcome_id}`);
  let result = find(master_question_object, (current)=> {
    return current['welcome_id'] == welcome_id; //&& current['current'] == '1';
  });
  return result;
};
...
module.exports.getAvatarPrompt = (question_number) => {
  return chain(avatar_list).sample().value();
};
