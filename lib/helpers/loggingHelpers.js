  ...
  console.log(`logging direct message event ${messages}`)
    ...
    _.each(messages.direct_message_mark_read_events, entry => {
      let payload = {
        ...
        twitter_account_name: messages.users[entry.sender_id].screen_name,
        twitter_account_id: entry.sender_id,
        receiving_account_id: entry.target.recipient_id,
        receiving_account: messages.users[entry.target.recipient_id].screen_name,
        dm_id: entry.last_read_event_id,
      };
      var process_payload = {
        dm_id: entry.last_read_event_id,
        read_at: moment(parseInt(entry.created_timestamp)).utc().format()
      };
      sidekiqConnection.enqueue(
        'Chirpify::Workers::ProcessDMReadEvent', [process_payload], {
          queue: 'logging'
        }
      );
      sidekiqConnection.enqueue(
