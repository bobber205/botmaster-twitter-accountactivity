...
redisClient.on('connect', function(err) {
  redisClient.getAsync(redisHelpers.getConfigurationKeyForHandle()).then((config_json) => {
...
class TwitterAccountActivityBot extends BaseBot {
  ...
  __createMountPoints(expressApp) {
    ...
    this.app.get('/facebook/webhook', (req, res) => {
    ...
    this.app.post('/facebook/webhook', (req, res) => {
              ...
              join(responded_status_key, responded_status_key_by_answer_value).then((values) => {
                ...
                if (is_not_bot_entry) {
                    if (Math.random() > 0.91) {
                      ...
                      message_to_user = `${message_to_user} You can't win the prize, but you should keep playing to improve your IQ score, which is currently ${user_iq_score}. We'll post the results and next question on our feed in 20 seconds!`;
                    } else if (responded_status_value != "1" && was_correct) {
                      message_to_user = `${message_to_user}  We'll post the results and next question on our feed in 20 seconds!`;
                    }
                } else {
